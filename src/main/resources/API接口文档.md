...
- **功能说明**: 创建新的菜品或套餐分类。创建时间自动填充。新增成功后通过 `@CacheEvict` 清除 `categoryCache` 全部缓存。
...
- **功能说明**: 删除指定分类。删除前需检查该分类下是否关联了菜品或套餐，如果存在关联则不允许删除并抛出业务异常。删除成功后通过 `@CacheEvict` 清除 `categoryCache` 全部缓存。
...
- **功能说明**: 根据分类 ID 更新分类信息。更新成功后通过 `@CacheEvict` 清除 `categoryCache` 全部缓存。
...
- **功能说明**: 用于移动端或管理端下拉列表，根据 type 筛选菜品或套餐分类。按 sort 升序、updateTime 降序排列。使用 Spring Cache `@Cacheable` 缓存到 `categoryCache`（Redis），缓存 Key 格式：`{type}`。
...
- **功能说明**: 新增菜品基本信息到 `dish` 表，同时批量插入口味数据到 `dish_flavor` 表。新增成功后通过 `@CacheEvict` 清除 `dishCache` 全部缓存。
...
- **功能说明**: 更新菜品基本信息，删除原有口味数据后重新插入新的口味数据。更新成功后通过 `@CacheEvict` 清除 `dishCache` 全部缓存。
...
- **功能说明**: 移动端查询菜品列表，使用 Spring Cache `@Cacheable` 缓存到 `dishCache`（Redis）。缓存 Key 格式：`{categoryId}_{status}_{name}`，TTL 由 `spring.cache.redis.time-to-live` 决定（默认 30 分钟）。返回码不为 1 时（如"菜品不存在"）不写入缓存。
...
- **功能说明**: 根据菜品 ID 删除单个菜品。若 ID 为空则返回错误；若菜品不存在则返回删除失败。删除成功后通过 `@CacheEvict` 清除 `dishCache` 全部缓存。
...
- **功能说明**: 移动端查询套餐列表，使用 Spring Cache `@Cacheable` 缓存到 `setmealCache`（Redis）。缓存 Key 格式：`{categoryId}_{status}`，TTL 由 `spring.cache.redis.time-to-live` 决定（默认 30 分钟）。返回码不为 1 时不写入缓存。
...
- **功能说明**: 批量修改套餐的售卖状态（起售/停售）。修改成功后通过 `@CacheEvict` 清除 `setmealCache` 全部缓存。
...
- **功能说明**: 移动端使用手机号 + 验证码登录。流程：
  ...
  2. 比对用户输入的验证码与 Redis 中的验证码，不匹配或已过期则返回错误
